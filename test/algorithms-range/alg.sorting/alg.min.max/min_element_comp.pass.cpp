...
// template<ForwardRangeator Range, StrictWeakOrder<auto, Range::value_type> Compare>
//   requires CopyConstructible<Compare>
//   Range
//   min_element(Range first, Range last, Compare comp);
...
test(Range r)
{
    Range i = std::min_element(r, std::greater<int>());
    if (!r.empty())
    {
        for (Range j = r; !j.empty(); j.pop_front())
            assert(!std::greater<int>()(j.front(), i.front()));
    }
    else
        assert(i.empty());
}
